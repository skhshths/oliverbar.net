...
- **`x913j1029jx1209x0f28j4f23fq28jc2q938jf.html`** — **Admin**. Password-gated (client-side check; see Security below). Once unlocked:
  - Toggle and rename the Interactive / Portfolio / Chat triggers. The Admin row itself is grayed out and can't be edited.
  - **Custom Redirects** — your own trigger words pointing to any path on this site or any external URL.
  - **Custom Pages** — paste raw HTML and it's hosted directly by the Worker at a generated URL (no Cloudflare Pages redeploy needed). Slugs can be nested paths (e.g. `test/about-us`) so pages can link to each other.
  - **Chat Moderation** — a "Clear All Chat Messages" button (asks for confirmation).
...
- **`x943j1029jx1209x0f28j4f23fq28jc2q938jf.html`** — **Global Chat**. Enter a display name, then post messages everyone currently on the page can see (polls the Worker every 3 seconds). No password — intentionally open to anyone who reaches it.
...
- **Chat has zero rate limiting or moderation.** See the API repo's README for what that means in practice and what stronger options exist (Turnstile, Durable Objects) if you want them later.
